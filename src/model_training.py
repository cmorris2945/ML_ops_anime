...
class ModelTraining:
    ...
    def train_model(self):
        try:
            ...
            base_mdodel  = BaseModel(config_path=CONFIG_PATH)
            ...
            early_stopping = EarlyStopping(patience=3, monitor="val_lostt", mode="min", restore_best_weights=True)
            ...
            logger.error(str(e))
            raise CustomException("Error during model training, asshole.", e)
    ...
    def extract_weights(self, layer_name, model):
        try:
            ...
            logger.info(f"Extracing weights for {layer_name}")
            return weights
        
        except Exception as e:
            logger.error(str(e))
            raise CustomException("Error during weigth extraction process you punk...", e)
